...
type Check = (r: { status: number; body: any }) => {
  ok: boolean;
  detail?: string;
};
...
async function test(
  ...
  if (skipReason) {
    skipped++;
    console.log(
      `  ${YELLOW}−${RESET} ${name} ${DIM}(skipped: ${skipReason})${RESET}`,
    );
    return;
  }
  try {
    ...
    if (ok) {
      passed++;
      console.log(
        `  ${GREEN}✓${RESET} ${name}${detail ? ` ${DIM}— ${detail}${RESET}` : ""}`,
      );
    } else {
      failed++;
      console.log(
        `  ${RED}✗${RESET} ${name} ${DIM}— ${detail || JSON.stringify(res.body).slice(0, 120)}${RESET}`,
      );
    }
  } catch (err) {
    failed++;
    console.log(
      `  ${RED}✗${RESET} ${name} ${DIM}— ${(err as Error).message}${RESET}`,
    );
  }
}
...
async function main() {
  ...
  console.log(
    `  ${DIM}Configured providers: ${available.join(", ") || "none"}${RESET}`,
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ...
      detail: `${body.providers?.length} total`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok:
        status === 200 &&
        body.providers?.every((p: any) => p.category === "financial"),
      detail: `${body.providers?.length} financial`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok:
        status === 200 &&
        body.providers?.every((p: any) => p.category === "ai"),
      detail: `${body.providers?.length} ai`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && Array.isArray(body.data),
      detail: `${body.data?.length} results`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && Array.isArray(body.data),
      detail: `${body.data?.length} resources`,
    }),
  );

  await test(
    "crisis_resources: type=hotline",
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && body.data?.length > 0,
      detail: `${body.data?.length} results`,
    }),
  );

  await test(
    "resource_search: substance abuse",
  ...
  await test(
    "coin_price: bitcoin + ethereum",
    () =>
      q("coingecko", "coin_price", {
        coins: "bitcoin,ethereum",
        currency: "usd",
      }),
    ({ status, body }) => ({
      ok: status === 200 && !!body.data?.bitcoin,
      detail: `BTC $${body.data?.bitcoin?.usd}`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ...
      detail: `${(body.data as any)?.coins?.length} trending`,
    }),
  );
  ...
  await test(
    "market_overview: top 5",
    () => q("coingecko", "market_overview", { limit: 5 }),
    ({ status, body }) => ({
      ok: status === 200 && Array.isArray(body.data) && body.data?.length === 5,
      detail: `${body.data?.[0]?.name} #1`,
    }),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && body.data?.current_price != null,
      detail: `AAPL $${body.data?.current_price}`,
    }),
    needs("finnhub"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && body.data?.current_price != null,
      detail: `NVDA $${body.data?.current_price}`,
    }),
    needs("finnhub"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && Array.isArray(body.data),
      detail: `${body.data?.length} articles`,
    }),
    needs("finnhub"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && body.data?.rate != null,
      detail: `1 USD = ${body.data?.rate} EUR`,
    }),
    needs("finnhub"),
  );
  ...
  await test(
    ...
    () =>
      q("environment", "current_weather", {
        city: "Nashville",
        units: "imperial",
      }),
    ({ status, body }) => ({
      ok: status === 200 && body.data?.temperature != null,
      detail: `${body.data?.temperature}°F, ${body.data?.description}`,
    }),
    needs("environment"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && body.data?.aqi != null,
      detail: `AQI ${body.data?.aqi} — ${body.data?.aqi_label}`,
    }),
    needs("environment"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ...
      detail: `"${body.data?.response?.trim().slice(0, 60)}"`,
    }),
    needs("claude"),
  );
  ...
  await test(
    ...
    () =>
      q("huggingface", "text_classification", {
        text: "I love this product, it is absolutely amazing!",
      }),
    ({ status, body }) => ({
      ok: status === 200 && !!body.data,
      detail: JSON.stringify(body.data).slice(0, 80),
    }),
    needs("huggingface"),
  );

  await test(
    "text_classification: negative",
    () =>
      q("huggingface", "text_classification", {
  ...
  await test(
    ...
    () =>
      q("geoapify", "geocode", {
        query: "Vanderbilt University Nashville TN",
        limit: 1,
      }),
    ({ status, body }) => ({
      ok: status === 200 && body.data?.length > 0,
      detail: body.data?.[0]?.display_name?.slice(0, 60),
    }),
    needs("geoapify"),
  );

  await test(
    "reverse_geocode: Nashville (36.16, -86.78)",
    () => q("geoapify", "reverse_geocode", { lat: 36.1627, lon: -86.7816 }),
    ({ status, body }) => ({
      ok: status === 200 && !!body.data?.display_name,
      detail: body.data?.display_name?.slice(0, 60),
    }),
    needs("geoapify"),
  );
  ...
  await test(
    ...
    ({ status, body }) => ({
      ok: status === 200 && !!body.data?.duration_readable,
      detail: `${body.data?.distance_km}, ${body.data?.duration_readable}`,
    }),
    needs("geoapify"),
  );
  ...
  console.log(`\n${"─".repeat(48)}`);
  console.log(
    `${BOLD}Results:${RESET}  ${GREEN}${passed} passed${RESET}  ·  ${RED}${failed} failed${RESET}  ·  ${YELLOW}${skipped} skipped${RESET}  ·  ${total} total`,
  );
  if (skipped > 0)
    console.log(`${DIM}Skipped = API key not set in backend env vars.${RESET}`);
