...
export class GeminiProvider implements Provider {
  info: ProviderInfo = {
    ...
    description: "Access Google Gemini AI models for text generation, analysis, and multimodal tasks.",
    availableActions: [
      {
        action: "chat",
        ...
        parameters: {
          ...
          model: { type: "string", description: "Model to use (default: gemini-1.5-flash)", required: false },
        },
      },
    ],
    requiresApiKey: true,
  ...
  async query(action: string, params: Record<string, unknown>): Promise<QueryResult> {
    ...
    if (!apiKey) return { success: false, error: "Google Gemini API key not configured" };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { success: false, error: `Gemini API error (${res.status}): ${text}` };
      }
