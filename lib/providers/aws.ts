...
export class AWSProvider implements Provider {
  info: ProviderInfo = {
    ...
    description:
      "Amazon Comprehend NLP service — detect sentiment, entities, key phrases, and language in text.",
    availableActions: [
      {
        action: "detect_sentiment",
        description:
          "Detect the sentiment (Positive, Negative, Neutral, Mixed) of a text",
        parameters: {
          text: {
            type: "string",
            description: "Text to analyze (max 5000 bytes)",
            required: true,
          },
          language: {
            type: "string",
      ...
      },
      {
        ...
        description:
          "Detect named entities (people, places, organizations, etc.) in text",
        parameters: {
          text: {
            type: "string",
  ...
  async query(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return {
          success: false,
          error: `AWS Comprehend error (${res.status}): ${errText}`,
        };
      }
