...
export class EnvironmentProvider implements Provider {
  info: ProviderInfo = {
    ...
    description: "Current weather conditions and air quality data for any location worldwide.",
    availableActions: [
      {
        action: "current_weather",
        description: "Get current weather for a city",
        parameters: {
          city: { type: "string", description: "City name (e.g., 'Nashville', 'London,UK')", required: true },
          units: { type: "string", description: "Units: metric, imperial, or standard (default metric)", required: false },
        },
      },
      {
        action: "air_quality",
  ...
  async query(action: string, params: Record<string, unknown>): Promise<QueryResult> {
    ...
    if (!apiKey) return { success: false, error: "OpenWeatherMap API key not configured" };
    ...
    switch (action) {
      case "current_weather": {
        ...
        if (!city) return { success: false, error: "Missing required parameter: city" };
        ...
        try {
          ...
          if (!res.ok) {
            ...
            return { success: false, error: `OpenWeatherMap error (${res.status}): ${text}` };
          }
      ...
      case "air_quality": {
        ...
        try {
          const res = await fetch(url);
          if (!res.ok) return { success: false, error: `OpenWeatherMap error (${res.status})` };
          const data = await res.json();
