...
export class GoogleMapsProvider implements Provider {
  info: ProviderInfo = {
    ...
    description: "Google Maps Platform — geocoding, place search, and directions.",
    availableActions: [
      {
        action: "geocode",
      ...
      },
      {
        ...
        parameters: {
          query: { type: "string", description: "Search query (e.g., 'coffee shops in Nashville')", required: true },
        },
      },
      {
        action: "directions",
        ...
        parameters: {
          origin: { type: "string", description: "Starting address or place", required: true },
          destination: { type: "string", description: "Destination address or place", required: true },
          mode: { type: "string", description: "Travel mode: driving, walking, bicycling, transit (default: driving)", required: false },
        },
      },
    ],
    requiresApiKey: true,
  ...
  async query(action: string, params: Record<string, unknown>): Promise<QueryResult> {
    ...
    switch (action) {
      case "geocode": {
        ...
        if (!address) return { success: false, error: "Missing required parameter: address" };
        const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&key=${apiKey}`;

        try {
          const res = await fetch(url);
          if (!res.ok) return { success: false, error: `Google Maps error (${res.status})` };
          const data = await res.json();
          if (data.status !== "OK") return { success: false, error: `Geocode error: ${data.status} — ${data.error_message || ""}` };
      ...
      case "directions": {
        ...
        if (!origin || !destination) return { success: false, error: "Missing required parameters: origin, destination" };
        ...
        try {
          ...
          if (!res.ok) return { success: false, error: `Google Maps error (${res.status})` };
          const data = await res.json();
          if (data.status !== "OK") return { success: false, error: `Directions error: ${data.status}` };
